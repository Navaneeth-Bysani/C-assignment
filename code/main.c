...
int main() {
	...
	//The following three functions fill the data in the files into the structures
	populateStudents();
	populateCourses();
	populateGrades();
	...
	//Keeps the program active until QUIT is entered
	while(1) {
		printf(">>>");
		gets(str);
		length = strlen(str);
...
int check(char *str, int length) {
	...
	if(!strcmp(cmd_1, "SHOW")) {
		//calls a function show(), that dispalys the data
		show(str,j);
...
//	printf("%s",filename);
	...
	if(str[j] == ';') {
		...
		if(!strcmp(filename,"csestudents")) {
			displayStudents();
			return;
		} else if(!strcmp(filename,"courses")) {
			displayCourses();
			return;
		} else if(!strcmp(filename,"grades")) {
			displayGrades();
			return;
		}
	}
